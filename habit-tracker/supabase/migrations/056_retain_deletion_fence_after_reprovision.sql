-- A fresh token may pass the deletion fence, but the tombstone must remain so
-- previously issued tokens continue to fail the same check after re-login.
...
BEGIN
  ...
  -- Do not consume the marker. Newer tokens pass; tokens issued at or before
  -- deletion remain denied even after a newer session has written successfully.
  RETURN NOT EXISTS (
    SELECT 1
