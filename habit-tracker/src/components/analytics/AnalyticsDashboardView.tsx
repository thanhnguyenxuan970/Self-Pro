...
export function AnalyticsDashboardView({ data, colors, isDark, language, range, reduceMotion, animationKey }: Props) {
  ...
  const metric = (value: number, previous: number, label: string, suffix: string, tone: 'primary' | 'gold' | 'ink') => <View style={[s.metric, tone === 'primary' && s.metricPrimary]} key={label}><Text style={[s.metricLabel, tone === 'primary' && s.metricPrimaryText]}>{label}</Text><Text style={[s.metricValue, tone === 'primary' && s.metricPrimaryText, tone === 'gold' && s.metricGold, tone === 'ink' && s.metricInk]}>{value}{suffix}</Text><Text style={[s.delta, tone === 'primary' && s.metricPrimaryText, delta(value, previous) < 0 && s.deltaBad]}>{delta(value, previous) >= 0 ? '▲' : '▼'}{Math.abs(delta(value, previous))}</Text></View>;
  return <View>
    <Text style={s.section}>{t.volume}</Text>
    <View style={[s.metrics, { marginBottom: 14 }]}>{metric(data.points, data.previousPoints, t.points, '', 'primary')}{metric(data.stars, data.previousStars, t.stars, '', 'gold')}{metric(data.daysAtGoal, data.previousDaysAtGoal, t.goal, `/${data.possibleDays}`, 'ink')}</View>
    <View style={s.card}>
