...
function ProgressRing({ value, color, track, reduceMotion, animationKey, label, textStyle }: { value: number; color: string; track: string; reduceMotion: boolean; animationKey: number; label: string; textStyle: object }) {
  ...
  return (
    <View accessibilityLabel={`${label}: ${value}%`} style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
      <Svg width={size} height={size} style={StyleSheet.absoluteFill}>
        ...
        <AnimatedCircle cx={34} cy={34} r={radius} fill="none" stroke={color} strokeWidth={8} strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} rotation="-90" origin="34, 34" />
      </Svg>
