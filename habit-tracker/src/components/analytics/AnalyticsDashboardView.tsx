...
export const AnalyticsDashboardView = React.memo(function AnalyticsDashboardView({ data, colors, isDark, language, range, reduceMotion, animationKey }: Props) {
  ...
  const metric = (value: number, previous: number, label: string, suffix: string, tone: 'primary' | 'gold' | 'ink', delay: number) => <View style={[s.metric, tone === 'primary' && s.metricPrimary]} key={label}><Text style={[s.metricLabel, tone === 'primary' && s.metricPrimaryText]} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72}>{label}</Text><AnimatedMetricValue value={value} suffix={suffix} style={[s.metricValue, tone === 'primary' && s.metricPrimaryText, tone === 'gold' && s.metricGold, tone === 'ink' && s.metricInk]} reduceMotion={reduceMotion} animationKey={animationKey} delay={delay} /><Text style={[s.delta, delta(value, previous) < 0 && s.deltaBad]}>{delta(value, previous) >= 0 ? '▲' : '▼'}{Math.abs(delta(value, previous))}</Text></View>;
  return <View>
