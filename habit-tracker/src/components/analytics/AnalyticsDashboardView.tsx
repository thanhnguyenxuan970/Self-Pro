...
export const AnalyticsDashboardView = React.memo(function AnalyticsDashboardView({ data, colors, isDark, language, range, reduceMotion, animationKey }: Props) {
  ...
  const metric = (value: number, previous: number, label: string, suffix: string, tone: 'primary' | 'gold' | 'ink', delay: number) => <View style={[s.metric, tone === 'primary' && s.metricPrimary]} key={label}><Text style={[s.metricLabel, tone === 'primary' && s.metricPrimaryText]}>{label}</Text><AnimatedMetricValue value={value} suffix={suffix} style={[s.metricValue, tone === 'primary' && s.metricPrimaryText, tone === 'gold' && s.metricGold, tone === 'ink' && s.metricInk]} reduceMotion={reduceMotion} animationKey={animationKey} delay={delay} /><Text style={[s.delta, delta(value, previous) < 0 && s.deltaBad]}>{delta(value, previous) >= 0 ? '▲' : '▼'}{Math.abs(delta(value, previous))}</Text></View>;
  return <View>
    ...
    <View style={s.card}><Text style={s.cardTitle}>{t.weekday}</Text>{data.weekday.map((day, index) => <View style={s.rhythmRow} key={day.label}><Text style={s.rhythmLabel}>{day.label}</Text><View style={s.rhythmTrack}><AnimatedRhythmFill value={day.value} max={weekdayMax} color={index === lowestWeekdayIndex ? C.ink2 : C.starGoldText} reduceMotion={reduceMotion} animationKey={animationKey} delay={index * 45} /></View><Text style={s.rhythmValue}>{day.value}</Text></View>)}
