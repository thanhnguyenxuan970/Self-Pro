...
function AnimatedBar({ value, max, color, style, reduceMotion, animationKey, delay = 0 }: { value: number; max: number; color: string; style: object; reduceMotion: boolean; animationKey: number; delay?: number }) {
  ...
  const fill = useFillAnimation({ reduceMotion, duration: 460, delay, useNativeDriver: true, deps: [animationKey, delay, reduceMotion, value] });
...
function AnimatedMetricValue({ value, suffix, style, reduceMotion, animationKey, delay = 0 }: { value: number; suffix: string; style: object; reduceMotion: boolean; animationKey: number; delay?: number }) {
  ...
  useEffect(() => {
    ...
    const animation = Animated.timing(progress, { toValue: value, delay, duration: 600, easing: Easing.out(Easing.cubic), useNativeDriver: false });
...
export const AnalyticsDashboardView = React.memo(function AnalyticsDashboardView({ data, colors, isDark, language, range, reduceMotion, animationKey }: Props) {
  ...
  const barWidth = range === 'M' ? 6 : range === 'Y' ? 14 : 8;
  ...
  const metric = (value: number, previous: number, label: string, suffix: string, tone: 'primary' | 'gold' | 'ink', delay: number) => <View style={[s.metric, tone === 'primary' && s.metricPrimary]} key={label}><Text style={[s.metricLabel, tone === 'primary' && s.metricPrimaryText]}>{label}</Text><AnimatedMetricValue value={value} suffix={suffix} style={[s.metricValue, tone === 'primary' && s.metricPrimaryText, tone === 'gold' && s.metricGold, tone === 'ink' && s.metricInk]} reduceMotion={reduceMotion} animationKey={animationKey} delay={delay} /><Text style={[s.delta, tone === 'primary' && s.metricPrimaryText, delta(value, previous) < 0 && s.deltaBad]}>{delta(value, previous) >= 0 ? '▲' : '▼'}{Math.abs(delta(value, previous))}</Text></View>;
  return <View>
    ...
    <View style={s.card}>
      ...
      <View style={[s.chart, { height: 235, paddingLeft: 26, paddingRight: 0 }]}>
        {[0, 50, 100].map((percent) => <View key={percent} pointerEvents="none" style={{ borderTopColor: C.line, borderTopWidth: 1, borderStyle: 'dotted', bottom: `${percent}%`, left: 26, position: 'absolute', right: 0 }} />)}
        <Text pointerEvents="none" style={{ color: C.muted, fontFamily: FontFamily.regular, fontSize: 9, left: 0, position: 'absolute', textAlign: 'right', top: -7, width: 20 }}>{chartMax}</Text>
        <Text pointerEvents="none" style={{ color: C.muted, fontFamily: FontFamily.regular, fontSize: 9, left: 0, position: 'absolute', textAlign: 'right', top: '47%', width: 20 }}>{chartMax / 2}</Text>
        <Text pointerEvents="none" style={{ color: C.muted, fontFamily: FontFamily.regular, fontSize: 9, left: 0, position: 'absolute', textAlign: 'right', bottom: -2, width: 20 }}>0</Text>
        {data.goal > 0 && <><View style={[s.goalLine, { bottom: `${(data.goal / chartMax) * 100}%`, left: 26 }]} /><Text style={[s.goalText, { backgroundColor: isDark ? C.surface2 : C.surface, bottom: `${(data.goal / chartMax) * 100}%`, left: 24, paddingHorizontal: 3, right: undefined }]}>{language === 'vi' ? `MỤC TIÊU ${data.goal}` : `GOAL ${data.goal}`}</Text></>}
        {data.bars.map((bar, index) => <View style={s.barCol} key={index}><View style={s.barPair}>{showPrevious && <AnimatedBar value={bar.previous} max={chartMax} color={C.faint} style={s.barPrevious} reduceMotion={reduceMotion} animationKey={animationKey} delay={index * 30} />}<AnimatedBar value={bar.current} max={chartMax} color={data.goal > 0 && bar.current < data.goal ? C.starGoldMuted : C.starGoldText} style={[s.barCurrent, !showPrevious && s.barCurrentSolo, { width: barWidth }]} reduceMotion={reduceMotion} animationKey={animationKey} delay={index * 30 + (showPrevious ? 70 : 0)} /></View><Text style={s.barLabel} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.6}>{bar.label}</Text></View>)}
      </View>
    ...
    <View style={s.card}><Text style={s.cardTitle}>{t.weekday}</Text>{data.weekday.map((day, index) => <View style={s.rhythmRow} key={day.label}><Text style={s.rhythmLabel}>{day.label}</Text><View style={s.rhythmTrack}><AnimatedRhythmFill value={day.value} max={weekdayMax} color={day.dayOfWeek === 6 ? C.ink2 : C.starGoldText} reduceMotion={reduceMotion} animationKey={animationKey} delay={index * 45} /></View><Text style={s.rhythmValue}>{day.value}</Text></View>)}
      <View style={s.rule} /><View style={s.chartHeader}><Text style={s.cardTitle}>{t.hour}</Text><Text style={s.legend}>{t.hourNote}</Text></View><View style={s.hours}>{data.hours.map((hour, index) => <View accessible accessibilityLabel={`${hour.label}: ${hour.value}`} style={s.hourCol} key={hour.label}><AnimatedBar value={hour.value} max={hourMax} color={C.starGoldText} style={s.hourBar} reduceMotion={reduceMotion} animationKey={animationKey} delay={index * 50} /><Text style={s.hourLabel}>{hour.label}</Text></View>)}</View>
    </View>
