  ...
  return <View>
    ...
    <View style={s.card}>
      ...
      <View style={[s.chart, { height: chartHeight, paddingTop: chartTop, paddingBottom: chartLabelHeight, paddingLeft: 34, paddingRight: 0 }]}>
        ...
        <View pointerEvents="none" style={[s.xAxisLabels, { height: chartLabelHeight }]}>{data.bars.map((bar, index) => <View style={s.xAxisColumn} key={`label-${index}`}><Text style={s.barLabel} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.6}>{chartLabel(bar.label)}</Text></View>)}</View>
      </View>
