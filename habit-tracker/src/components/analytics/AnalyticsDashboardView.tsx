...
const copy = (vi: boolean) => vi ? {
  volume: 'KHỐI LƯỢNG', points: 'ĐIỂM', stars: 'SAO', goal: 'NGÀY HOẠT ĐỘNG', chart: 'Điểm theo ngày', thisWeek: 'Tuần này', last: 'Trước đó', consistency: 'ĐỀU ĐẶN', rhythm: 'NHỊP ĐỘ', weekday: 'Theo thứ', hour: 'Theo giờ', composition: 'THÀNH PHẦN', share: 'TỶ LỆ', logs: 'LƯỢT', all: 'Tất cả thời gian', goalNote: (n: number, days: number, peak: string, value: number) => `${n} / ${days} ngày hoạt động · đỉnh ${peak}, ${value} điểm`, monthNote: (n: number, days: number, zero: number, best: number) => `${n} / ${days} ngày hoạt động · ${zero} ngày 0 điểm · cao nhất ${best} điểm`, yearNote: (cleared: number, total: number, goal: string, change: number | null, from: string) => `${cleared} / ${total} tháng đạt mốc ${goal} điểm · ${change === null ? 'chưa đủ dữ liệu xu hướng' : change > 0 ? `tăng ${change}% từ ${from}` : change < 0 ? `giảm ${Math.abs(change)}% từ ${from}` : `không đổi từ ${from}`}`, hourNote: (start: string, end: number, share: number) => `đỉnh ${start}–${end}h · ${share}% số điểm`, hourSummary: (weakest: string, value: number, strongest: string, ratio: string | null, start: string, end: number, share: number) => `${weakest} là ngày có tổng điểm thấp nhất (${value} điểm)${ratio ? ` · ${strongest} cao hơn ${ratio} lần` : ''} · ${share}% số điểm tập trung trong khung ${start}–${end}h.`
} : {
  volume: 'VOLUME', points: 'POINTS', stars: 'STARS', goal: 'ACTIVE DAYS', chart: 'Points per day', thisWeek: 'This week', last: 'Last', consistency: 'CONSISTENCY', rhythm: 'RHYTHM', weekday: 'By weekday', hour: 'By hour', composition: 'COMPOSITION', share: 'SHARE', logs: 'N', all: 'All time', goalNote: (n: number, days: number, peak: string, value: number) => `${n} of ${days} active days · ${peak} ${value} pts`, monthNote: (n: number, days: number, zero: number, best: number) => `${n} of ${days} active days · ${zero} zero-days · best day ${best} pts`, yearNote: (cleared: number, total: number, goal: string, change: number | null, from: string) => `${cleared} of ${total} months cleared ${goal} pts · ${change === null ? 'not enough trend data' : change > 0 ? `up ${change}% since ${from}` : change < 0 ? `down ${Math.abs(change)}% from ${from}` : `flat from ${from}`}`, hourNote: (start: string, end: number, share: number) => `peak ${start}–${end}h · ${share}% of points`, hourSummary: (weakest: string, value: number, strongest: string, ratio: string | null, start: string, end: number, share: number) => `${weakest} is your lowest-output day (${value} pts)${ratio ? ` · ${strongest} is ${ratio}× higher` : ''} · ${share}% of points land in ${start}–${end}h.`
};
...
function AnalyticsMetricsRow({ data, language, reduceMotion, animationKey, s }: {
  ...
  const metric = (value: number, previous: number, label: string, suffix: string, tone: 'primary' | 'gold' | 'ink', delay: number) => <View style={[s.metric, tone === 'primary' && s.metricPrimary]} key={label}><Text style={[s.metricLabel, tone === 'primary' && s.metricPrimaryText]} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.85}>{label}</Text><AnimatedMetricValue value={value} suffix={suffix} style={[s.metricValue, tone === 'primary' && s.metricPrimaryText, tone === 'gold' && s.metricGold, tone === 'ink' && s.metricInk]} reduceMotion={reduceMotion} animationKey={animationKey} delay={delay} /><Text style={[s.delta, tone === 'primary' && s.metricPrimaryText, delta(value, previous) < 0 && s.deltaBad]}>{delta(value, previous) >= 0 ? '▲' : '▼'}{Math.abs(delta(value, previous))}</Text></View>;
  return <>
...
export const AnalyticsDashboardView = React.memo(function AnalyticsDashboardView({ data, colors, isDark, language, range, reduceMotion, animationKey }: Props) {
  const C = useMemo(() => ({ ...colors, starGoldText: colors.primaryText, starGoldMuted: colors.primaryLine }), [colors]);
