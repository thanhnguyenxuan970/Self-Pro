...
import { Modal, ScrollView, StyleSheet, Text, TouchableOpacity, View, Animated, Easing, useWindowDimensions } from 'react-native';
...
import { HEATMAP_DATE_CONTROL_MIN_SIZE, HeatmapDay, buildHeatmapWeeks, heatmapShades, stepHeatmapAccessibilityDate } from '../utils/heatmap';
...
const AnimatedWeeks = React.memo(function AnimatedWeeks({ weeks, styles, shades, today, pointsByDate, onSelect, animKey, reduceMotion, todayBorderColor, accessibilityCopy }: {
  ...
  accessibilityCopy: { label: string; previousDay: string; nextDay: string; openDetails: string; points: (value: number) => string };
}) {
  const accessibleDates = useMemo(() => weeks.flat().map(cell => cell.date).filter(Boolean), [weeks]);
  const [accessibleDate, setAccessibleDate] = useState(() => accessibleDates.includes(today) ? today : accessibleDates.at(-1) ?? '');
  useEffect(() => {
    if (!accessibleDates.includes(accessibleDate)) {
      setAccessibleDate(accessibleDates.includes(today) ? today : accessibleDates.at(-1) ?? '');
    }
  }, [accessibleDate, accessibleDates, today]);
  ...
  return <View
    ...
    accessibilityLabel={accessibilityCopy.label}
    accessibilityValue={{ text: `${accessibleDate}: ${accessibilityCopy.points(pointsByDate.get(accessibleDate) ?? 0)}` }}
    accessibilityActions={[
      { name: 'decrement', label: accessibilityCopy.previousDay },
      { name: 'increment', label: accessibilityCopy.nextDay },
      { name: 'activate', label: accessibilityCopy.openDetails },
    ]}
    onAccessibilityAction={({ nativeEvent }) => {
      if (nativeEvent.actionName === 'activate') {
        if (accessibleDate) onSelect(accessibleDate);
        return;
      }
      if (nativeEvent.actionName === 'increment' || nativeEvent.actionName === 'decrement') {
        setAccessibleDate(current => stepHeatmapAccessibilityDate(
          accessibleDates,
          current,
          nativeEvent.actionName === 'increment' ? 'next' : 'previous',
        ));
      }
    }}
  ><View style={styles.weeksRow}>{weeks.map((week, i) => (
    ...
    <View style={styles.dateControls} accessible={false}>
      <TouchableOpacity
        style={styles.dateControlButton}
        onPress={() => setAccessibleDate(current => stepHeatmapAccessibilityDate(accessibleDates, current, 'previous'))}
        accessibilityRole="button"
        accessibilityLabel={accessibilityCopy.previousDay}
      ><Text style={styles.dateControlGlyph}>‹</Text></TouchableOpacity>
      <TouchableOpacity
        style={styles.dateControlValue}
        onPress={() => accessibleDate && onSelect(accessibleDate)}
        accessibilityRole="button"
        accessibilityLabel={accessibilityCopy.openDetails}
      ><Text style={styles.dateControlText} numberOfLines={1}>{accessibleDate}</Text></TouchableOpacity>
      <TouchableOpacity
        style={styles.dateControlButton}
        onPress={() => setAccessibleDate(current => stepHeatmapAccessibilityDate(accessibleDates, current, 'next'))}
        accessibilityRole="button"
        accessibilityLabel={accessibilityCopy.nextDay}
      ><Text style={styles.dateControlGlyph}>›</Text></TouchableOpacity>
    </View>
  </View>;
});
...
export const HomeHeatmap = React.memo(function HomeHeatmap({ days, streak, goal, colors, todayPoints, rankEmoji, lifetimeStars, rankName, streakRef, scoringGuideVisible = false, onScoringGuideClose, boostVisual = null }: Props) {
  ...
  const heatmapColumnPitch = layout.cellSize + layout.cellGap;
      ...
      <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: false })}>
        <View>
          <View style={styles.months}>{weeks.map((week, i) => {
            ...
            return month ? <Text key={i} style={[styles.month, { left: i * heatmapColumnPitch }]}>{month}</Text> : null;
          })}</View>
          <AnimatedWeeks
            ...
            accessibilityCopy={{
              label: t.heatmapAccessibilityLabel,
              previousDay: t.heatmapPreviousDay,
              nextDay: t.heatmapNextDay,
              openDetails: t.heatmapOpenDayDetails,
              points: t.ptsLabel,
            }}
          />
        </View>
    ...
    <Modal visible={scoringGuideVisible} transparent animationType={reduceMotion ? 'none' : 'fade'} onRequestClose={onScoringGuideClose} statusBarTranslucent navigationBarTranslucent>
      <View style={styles.legendModal}>
        ...
        <View style={styles.scoringSheetContainer}>
          ...
          <View style={styles.scoringDivider} />
          {t.scoringStarRules.map((rule) => <View key={rule.title} style={styles.legendItem}><Text style={styles.starBadge}>★</Text><View style={styles.scoringCopy}><Text style={styles.legendItemText}>{rule.title}</Text><Text style={styles.scoringExample}>{rule.example}</Text></View></View>)}
          </ScrollView>
        </View>
      </View>
    </Modal>
...
function makeStyles(C: AppColors, L: HeatmapLayout) {
  return StyleSheet.create({
    ...
    gridRow: { flexDirection: 'row' }, rail: { width: 24, marginTop: 24, gap: L.cellGap }, dayLabel: { height: 17, color: C.faint, fontSize: 11, lineHeight: 17, fontFamily: FontFamily.semiBold }, months: { height: 20, position: 'relative' }, month: { position: 'absolute', width: 36, color: C.faint, fontSize: 11, lineHeight: 14, fontFamily: FontFamily.semiBold }, weeks: { gap: 4 }, weeksRow: { flexDirection: 'row', gap: L.cellGap }, week: { gap: L.cellGap }, cell: { width: L.cellSize, height: L.cellSize, borderRadius: 4, overflow: 'hidden' }, emptyCell: {}, todayCell: { borderWidth: 1.6, borderStyle: 'solid', borderColor: C.primary }, dateControls: { minHeight: HEATMAP_DATE_CONTROL_MIN_SIZE, flexDirection: 'row', alignSelf: 'flex-end', borderWidth: 1, borderColor: C.line, borderRadius: Radii.pill, overflow: 'hidden' }, dateControlButton: { width: HEATMAP_DATE_CONTROL_MIN_SIZE, minHeight: HEATMAP_DATE_CONTROL_MIN_SIZE, alignItems: 'center', justifyContent: 'center', backgroundColor: C.surface2 }, dateControlValue: { minWidth: 132, minHeight: HEATMAP_DATE_CONTROL_MIN_SIZE, paddingHorizontal: Spacing.sm, alignItems: 'center', justifyContent: 'center', borderLeftWidth: 1, borderRightWidth: 1, borderColor: C.line, backgroundColor: C.surface }, dateControlGlyph: { color: C.primaryText, fontSize: 26, lineHeight: 30, fontFamily: FontFamily.bold }, dateControlText: { color: C.inkDark, fontSize: 13, fontFamily: FontFamily.bold },
