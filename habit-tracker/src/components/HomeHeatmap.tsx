...
import { HEATMAP_CELL_HIT_SLOP, HeatmapDay, buildHeatmapWeeks, heatmapShades } from '../utils/heatmap';
      ...
      {week.map((cell, j) => {
        ...
        return (
          <AnimatedTouchable
            ...
            // Keep the compact visual cell while exposing an effective 44pt
            // target for touch and accessibility users.
            onPress={() => onSelect(cell.date)} hitSlop={HEATMAP_CELL_HIT_SLOP}
...
function makeStyles(C: AppColors, L: HeatmapLayout) {
  return StyleSheet.create({
    ...
    gridRow: { flexDirection: 'row' }, rail: { width: 24, marginTop: 24, gap: L.cellGap }, dayLabel: { height: 17, color: C.faint, fontSize: 11, lineHeight: 17, fontFamily: FontFamily.semiBold }, months: { height: 20, position: 'relative' }, month: { position: 'absolute', width: 36, color: C.faint, fontSize: 11, lineHeight: 14, fontFamily: FontFamily.semiBold }, weeks: { flexDirection: 'row', gap: L.cellGap }, week: { gap: L.cellGap }, cell: { width: L.cellSize, height: L.cellSize, borderRadius: 4, overflow: 'hidden' }, emptyCell: {}, todayCell: { borderWidth: 1.6, borderStyle: 'solid', borderColor: C.primary },
