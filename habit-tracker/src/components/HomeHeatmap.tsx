...
type Props = {
  ...
  rankEmoji?: string; yearStars?: number; rankName?: string; streakRef?: (node: View | null) => void; streakOnLayout?: () => void;
...
export const HomeHeatmap = React.memo(function HomeHeatmap({ days, streak, goal, colors, todayPoints, rankEmoji, yearStars, rankName, streakRef, streakOnLayout, scoringGuideVisible = false, onScoringGuideClose, boostVisual = null }: Props) {
  ...
  return <>
    ...
    <View style={styles.rewardRow}>
      <View ref={streakRef} onLayout={streakOnLayout} style={[styles.rewardPill, boostActive && palette ? { borderColor: palette.border, borderWidth: 1 } : null]}>
        <Text style={styles.rewardText}>{rankName ? `🔥 ${streak} · ★ ${Math.round(yearStars ?? 0)} › ${rankEmoji} ${rankName}` : `🔥 ${streak}`}</Text>
      </View>
