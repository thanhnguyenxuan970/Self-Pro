...
export function useLogTask(userId: number) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Persist first use immediately; delay only the visible native prompt so
      // streak celebrations and the add-activity sheet can finish first.
      recordFirstUse().catch(() => {});
