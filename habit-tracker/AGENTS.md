...
| `duplicate key value violates unique constraint "users_email_partial_key"` or HTTP `500` from `/auth/v1/token` | Concurrent Google ID-token sign-ins, redundant fresh-session exchange, or a transient GoTrue auth exchange failure | Reuse a fresh same-account session, share one in-flight token exchange, retry one bounded auth 5xx/insert race, and surface non-race auth failures before publishing local auth state |
