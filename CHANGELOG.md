...
### Fixed
- **Google sign-in race hardening**: malformed identities and missing ID tokens now fail closed before local account mutation; same-account exchanges remain serialized with bounded duplicate-key/5xx retry, finite-expiry/email/Google-subject checks, one process-wide session lease for protected RPCs, cancellation-safe native restore, token-owned cleanup, atomic local user/category seeding, and localized failure copy.
