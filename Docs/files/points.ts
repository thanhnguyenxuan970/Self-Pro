// points.ts — star calculation for duration-based habits
// Rule: every 30 min = 1★ up to 2h, then half-rate beyond (anti-inflation).

export interface PointConfig {
  minutesPerBlock: number;     // size of one earning block
  starsPerFullBlock: number;   // stars per block within the full-rate zone
  fullRateBlocks: number;      // how many blocks earn the full rate (4 = first 2h)
  reducedRateDivisor: number;  // blocks-per-star beyond full rate (2 = half-rate)
  dailyCapStars?: number;      // optional hard ceiling per activity per day
  hardCap?: boolean;           // true = no stars at all past fullRateBlocks
}

export const DEFAULT_POINT_CONFIG: PointConfig = {
  minutesPerBlock: 30,
  starsPerFullBlock: 1,
  fullRateBlocks: 4,      // 4 × 30min = 2h at full rate
  reducedRateDivisor: 2,  // then 1★ per 2 extra blocks (= per extra hour)
  hardCap: false,
};

/**
 * Stars earned for a session of `durationMin` minutes.
 *   full-rate zone (≤2h): 1★ / 30min
 *   beyond:               1★ / 60min  (half-rate)  — or 0 if hardCap
 *
 *   30m→1  1h→2  1.5h→3  2h→4  2.5h→4  3h→5  4h→6  8h→10
 */
export function computeStars(
  durationMin: number,
  cfg: PointConfig = DEFAULT_POINT_CONFIG,
): number {
  if (durationMin <= 0) return 0;

  const blocks = Math.floor(durationMin / cfg.minutesPerBlock);
  const fullBlocks = Math.min(blocks, cfg.fullRateBlocks);
  const extraBlocks = Math.max(0, blocks - cfg.fullRateBlocks);

  let stars = fullBlocks * cfg.starsPerFullBlock;
  if (!cfg.hardCap) {
    stars += Math.floor(extraBlocks / cfg.reducedRateDivisor) * cfg.starsPerFullBlock;
  }

  if (cfg.dailyCapStars != null) stars = Math.min(stars, cfg.dailyCapStars);
  return stars;
}

/** Minutes → "1.5h" / "45m" / "3h 30m" for chip + summary labels. */
export function formatDuration(min: number): string {
  if (min < 60) return `${min}m`;
  const h = Math.floor(min / 60);
  const m = min % 60;
  return m === 0 ? `${h}h` : `${h}h ${m}m`;
}
